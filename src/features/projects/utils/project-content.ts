...
export function createEmptyProjectContent(): OutputData {
  return {
    time: Date.now(),
    blocks: [
      {
        ...
        type: "header",
        data: { text: "Project story", level: 2 },
      },
      {
        id: "project-intro-copy",
        type: "paragraph",
        data: {
          text: "Shape the story, add media, and prepare the final studio presentation here.",
        },
      },
    ],
  };
...
export function normalizeProjectContent(content?: OutputData | null): OutputData {
  const baseContent = content?.blocks ? content : createEmptyProjectContent();
...
export function serializeProjectContent(content: OutputData): OutputData {
...
export function getProjectContentSubmitError(content: OutputData) {
  const normalized = normalizeProjectContent(content);
