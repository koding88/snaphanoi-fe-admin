...
import { COUNTRY_OPTIONS, DEFAULT_COUNTRY_CODE, getCountryByCode } from "@/lib/constants/countries";
import { faChevronDown, faMagnifyingGlass, faXmark } from "@/lib/icons/fa";
...
export function CountrySelect({ value, onChange, className, id }: CountrySelectProps) {
  ...
  const filteredCountries = useMemo(() => {
    ...
    return COUNTRY_OPTIONS.filter((country) =>
      `${country.name} ${country.code}`.toLowerCase().includes(keyword),
    );
  }, [search]);
  ...
  useEffect(() => {
    ...
    function updatePopupPosition() {
      ...
      const maxHeight = Math.max(220, Math.min(420, preferAbove ? spaceAbove - 10 : spaceBelow));
      ...
      >
        <span className="flex min-w-0 items-center gap-3">
          ...
          <span className="min-w-0">
            ...
            <span className="block text-xs text-muted-foreground">{selectedCountry.code}</span>
          </span>
        </span>
