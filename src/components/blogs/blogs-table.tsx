        ...
        <div className="border-t border-border/10">
          ...
          <div>
            {blogs.map((blog) => {
              ...
              const secondaryNames = getBlogSecondaryNameLines(blog.name);
                ...
                >
                  <div className="space-y-1">
                    ...
                    {secondaryNames.length > 0 ? (
                      <p className="truncate text-xs uppercase tracking-[0.14em] text-[--color-brand-muted]">
                        {secondaryNames
                          .map(
                            (entry) =>
                              `${entry.locale.toUpperCase()}: ${entry.value}`,
                          )
                          .join(" · ")}
                      </p>
                    ) : null}
                    <p className="text-sm text-muted-foreground">
                      {t("by", {
                        name: formatCreatorDisplayName(blog.createdBy.name),
                      })}
                    </p>
        ...
        title={
          pendingAction?.type === "delete"
            ? t("dialogs.deleteTitle", {
                name: pendingAction
                  ? getBlogPrimaryName(pendingAction.blog.name)
                  : "",
              })
        ...
        extra={
          pendingAction ? (
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                {formatCreatorDisplayName(pendingAction.blog.createdBy.name)}
              </p>
            </div>
          ) : null
        }
