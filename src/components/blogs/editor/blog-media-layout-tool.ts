...
import type {
  BlogEditorImageUploader,
  BlogEditorMediaItem,
  BlogEditorMediaLayoutData,
  BlogEditorUploadResult,
} from "@/components/blogs/editor/blog-editor.types";
import { BlogMediaTile } from "@/components/blogs/editor/blog-media-tile";

const EditableGrid = WidthProvider(GridLayout);

type BlogMediaLayoutToolConfig = {
  onChange?: (data: BlogEditorMediaLayoutData) => void;
...
class BlogMediaLayoutTool {
  private data: BlogEditorMediaLayoutData;
  private config: BlogMediaLayoutToolConfig;
  ...
  constructor({
    ...
    data?: Partial<BlogEditorMediaLayoutData>;
    config: BlogMediaLayoutToolConfig;
  }) {
  ...
  save(): BlogEditorMediaLayoutData {
    return {
...
export default BlogMediaLayoutTool;
